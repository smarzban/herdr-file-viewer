...
| `?` (Shift+`/`) | Open the **help overlay**, with **What's New** selected first: project-spotlight and available-update detail, full released history, then Keybindings, Settings, and About; `Esc` / `q` closes it |
| `u` | Dismiss the whole status row for the current session. An update returns next session while you are still behind; exact spotlight content is remembered until that content changes, and `u` does not remove its body from What's New |
