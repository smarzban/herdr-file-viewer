...
the viewer — the wrong default for an open you initiated. If the user explicitly asks for a popup,
also pass `--env HERDR_FILE_VIEWER_PLACEMENT=popup`: that marker is how the viewer knows it is in a
popup and not a pane, and without it the `Z` full-screen key asks herdr to zoom the pane underneath,
