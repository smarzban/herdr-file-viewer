...
| `view_policy` | A pure decision: which view mode a file gets (document → rendered document, changed → diff, markdown → rendered, else → syntax content) and the cycle order. |
| `document` | A pure classifier: recognize a binary document by extension (docx/odt/pdf/pptx/xlsx) and describe the external converter for it (a stdout converter like `pandoc`/`pdftotext`, or a temp-file converter like `libreoffice`). No I/O. |
| `render` | Produce the content-pane text: classify the file, delegate styling to an external CLI, and **neutralize escape sequences** before display. Documents take a separate path — a `document` converter (given the file path) produces text that is then rendered through the markdown delegate. |
