...
| `tree` | The rooted, `.gitignore`-aware file tree: filters (gitignored, changed-only, hidden/dotfiles), cursor, expansion, status markers, and the `]` / `[` changed-file jump. Optionally folds a chain of single-child directories into one row (`compact_dirs`), memoizing its directory listings while it does so — a folded row has to look inside a collapsed directory, which is too much walking to repeat per frame — and re-reading them wherever the controller re-reads git. |
