...
| `preview_layout` | Pure responsive geometry for the tree, pinned preview, and active preview. It keeps the preview divider distinct from the tree divider, applies the 20–80% pinned share, and falls back to the focused region when either preview misses its 40-column floor. |
| `focus_policy` | Pure focus cycle and action-target policy. With a pin it routes `Tab` through tree → pinned → active, gives pinned in-file navigation its own target, and rejects live-selection actions from the frozen reference. |
