...
| `tree` | The rooted, `.gitignore`-aware file tree: filters (gitignored, changed-only, hidden/dotfiles), cursor, expansion, status markers, and the `]` / `[` changed-file jump. Also the **session view** presentation: the session file set synthesized through the same emitter the changed-only tree uses (git decorations intact), then a non-selectable separator row and the outside-root section — absolute-path groups with chains always folded and home `~`-abbreviated; the cursor snaps off the separator, and `reveal` relaxes the view for a non-member target exactly like a filter. Optionally folds a chain of single-child directories into one row (`compact_dirs`). A folded row has to look inside a **collapsed** directory, which the tree never opens otherwise, so foldability is answered by a two-entry probe rather than a listing and the answer is memoized — re-probed wherever the controller re-reads git. Listings stay uncached, so a compacted frame reads exactly the directories an uncompacted one does. |
...
| `session` | The Session Transcript Reader behind the session view (ADR-0011): locate Claude Code's per-project transcripts (`~/.claude/projects/<slug>`), incrementally follow the current one **and its subagent transcripts** (Task-tool sidechains are separate files under `<session-id>/subagents/`), and fold the deterministic signals — file-tool results, user `@`-mentions — into the session file set, split in-root (root-relative, changed-set-shaped) vs outside-root (absolute), each member carrying its strongest category (created ≻ updated ≻ mentioned). Reads are append-only tails, chunk-bounded per poll so a huge transcript streams in without blocking input; a shrunk or vanished file triggers a clean rebuild of the merged set. Read-only and fail-soft throughout: unknown shapes are skipped, all failures degrade to an empty set. |
